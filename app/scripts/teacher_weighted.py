# teacher_weighted.py — YOLOv8 tréner globális loss-súlyokkal (box/cls/dfl) + metrikák + mintapredikciók
# by Selene ✨
...
def read_and_validate_yaml(path: Path) -> dict:
    ...
    if isinstance(names, dict):
        # kulcsok számmá konvertálása szerinti rendezéssel
...
def write_cfg(model: str, data: str, args) -> Path:
    ...
    Itt már beírjuk a globális loss-súlyokat is (box, cls, dfl, fl_gamma),
    mert a jelenlegi Ultralytics build ezeket szépen kezeli.
    """
    cfg = {
        "task": "detect",
        "mode": "train",
    ...
    # ---- opt/hyp kulcsok (globális súlyok) ----
    ...
    if args.box is not None:
        cfg["box"] = args.box      # box loss súly
    if args.cls is not None:
        cfg["cls"] = args.cls      # cls loss súly
    if args.dfl is not None:
        cfg["dfl"] = args.dfl      # distribution focal loss súly
    if args.fl_gamma is not None:
        cfg["fl_gamma"] = args.fl_gamma  # focal gamma (cls-re)
...
def main():
    p = argparse.ArgumentParser("YOLOv8 train – cfg alapú tréner + globális loss-súlyok + metrikák + mintapred")
    ...
    # ---- GLOBÁLIS LOSS-SÚLYOK ----
    p.add_argument("--box-w", dest="box", type=float, default=None, help="box loss súlya (alap ~7.5)")
    p.add_argument("--cls-w", dest="cls", type=float, default=None, help="cls loss súlya (alap ~0.5)")
    p.add_argument("--dfl-w", dest="dfl", type=float, default=None, help="DFL loss súlya (alap ~1.5)")
    p.add_argument("--fl-gamma", dest="fl_gamma", type=float, default=None, help="Focal gamma (0 = sima BCE)")
    ...
    # infó a loss-súlyokról
    for k in ["box", "cls", "dfl", "fl_gamma"]:
        v = getattr(args, k, None)
        if v is not None:
            print(f"[INFO] Globális loss-súly: {k} = {v}")
