...
\begin{frame}{Áttekintés}
  ...
  \begin{minipage}{0.72\textwidth}
    \begin{enumerate}
      ...
      \item \textbf{Probléma és motiváció}
      \item \textbf{Adathalmaz és tanítási stratégia}
      \item \textbf{Modell teljesítménye}
      \item \textbf{Rendszerarchitektúra}
      \item \textbf{Összefoglalás és jövőbeli irányok}
      \item \textbf{Demó}
    \end{enumerate}
  \end{minipage}
\end{frame}
  ...
  \begin{columns}[T]
    \column{0.48\textwidth}
      \begin{alertblock}{Magas súlyosság – azonnali beavatkozás}
        ...
        \end{itemize}
        \vspace{0.3em}
        \textbf{$\rightarrow$ E-mail értesítés}
      \end{alertblock}
    \column{0.48\textwidth}
      \begin{exampleblock}{Alacsony súlyosság – naplózás}
        \vspace{0.3em}
        \centering
        + 5 további hibaosztály
        \vspace{0.3em}
        \textbf{$\rightarrow$ Csak Firestore naplózás}
      \end{exampleblock}
  \end{columns}
\end{frame}
...
% ── Slide 6: YOLOv8x + kétfázisú tanítás ─────────────────────────────────────
  ...
  \begin{columns}[T]
    ...
    \column{0.46\textwidth}
      ...
      \begin{exampleblock}{Konfidenciaküszöb: 35\%}
        ...
        Makró F1 (összesített): \textbf{0.88}
      \end{exampleblock}
    \end{columns}
\end{frame}

% ── Slide 7: Tanítási görbék ──────────────────────────────────────────────────
...
% ── Slide 8: PR görbe + Konfúziós mátrix ─────────────────────────────────────
\begin{frame}{Precision-Recall görbe \& Konfúziós mátrix}
  \vspace*{-1.3em}
  \hspace*{-0.5cm}%
  \includegraphics[width=0.5\paperwidth,height=\textheight,keepaspectratio]{results/y8x_phase2_12803/confusion_matrix_normalized.png}%
  \includegraphics[width=0.5\paperwidth,height=\textheight,keepaspectratio]{results/y8x_phase2_12803/BoxPR_curve.png}%
\end{frame}

% ── Slide 9: Validációs predikciók ────────────────────────────────────────────
...
% ── Slide 10: Összesített teljesítmény ───────────────────────────────────────
...
\begin{frame}{Az adatfolyam lépései}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Raspberry Pi 4B} (3 webkamera) $\rightarrow$ \texttt{frame-extractor}:\\
          2 fps/kamera, 1280×960 JPEG $\rightarrow$ Pub/Sub \texttt{frames-in}
...
\begin{frame}{Élő dashboard}
  \centering
  \includegraphics[width=0.95\textwidth,keepaspectratio]{literature/full_dashboard.png}

  \vspace{0.35em}
  {\footnotesize 3 élő WebRTC kamerastream + valós idejű detekciós napló (Firestore onSnapshot)}
\end{frame}

% ── Slide 14: Összefoglalás + Jövőbeli irányok ───────────────────────────────
...
% ── Slide 15: Demó ────────────────────────────────────────────────────────────
\begin{frame}{Élő rendszer – videó és e-mail riasztás}
  \centering
  \colorbox{lightbg}{%
    \begin{minipage}[c][1.4cm][c]{0.85\linewidth}
      \centering
      {\large\textcolor{primary}{$\blacktriangleright$}}\quad
      {\normalsize\textbf{Demo videó}}
    \end{minipage}%
  }

  \vspace{0.5em}
  \includegraphics[width=0.88\textwidth,keepaspectratio]{demo/emailkep.png}\\[0.3em]
  {\small\textcolor{alertred}{\textbf{E-mail riasztás}} – magas súlyosságú hiba észlelve}
\end{frame}
